...
async function loadPhrases() {
    try {
        ...
        conversationalPhrases = phrasesModule.conversationalPhrases || [];
        console.log(`Loaded ${singleWordPhrases.length + twoWordPhrases.length + threeWordPhrases.length + conversationalPhrases.length} phrases`);
    } catch (error) {
...
function updatePhrasesArray() {
    ...
    if (conversationalPhrases.length > 0) {
        phrases = phrases.concat(conversationalPhrases);
    }
...
const useExternalApi = false; // Disabled - using only local phrases
const externalApiFrequency = 0; // 0% chance to use external API

// External APIs for fetching words - DISABLED
const apis = {
    // API configurations removed - using only local phrases
};
...
// Get a phrase (from local list only)
const getPhrase = async () => {
    // Using only local phrases
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Schedule first update after 5 seconds
    updateTimer = setTimeout(updateDisplay, 5000);
    
    // Add invisible tap zones for manual navigation
    setupTapZones();
});

// History tracking for previous phrases
let phraseHistory = [];
let currentPhraseIndex = -1;

// Setup invisible tap zones
function setupTapZones() {
    const body = document.body;
    
    // Add click/touch event listener to the entire body
    body.addEventListener('click', handleTapNavigation);
    body.addEventListener('touchend', handleTapNavigation);
}
...
function handleTapNavigation(event) {
    ...
    const tapX = event.clientX || (event.changedTouches && event.changedTouches[0].clientX);
    
    // Define tap zones
    const leftZone = screenWidth * 0.25;  // Left 25% of screen
    const rightZone = screenWidth * 0.75; // Right 75% of screen (so right 25%)
    
    if (tapX < leftZone) {
        // Left tap - show feedback and go to previous phrase
        showTapFeedback('left');
        goToPreviousPhrase();
    } else if (tapX > rightZone) {
        // Right tap - show feedback and go to next phrase
        showTapFeedback('right');
        goToNextPhrase();
    }
    // Center 50% does nothing - preserves contemplation
}

// Show tap feedback animation
function showTapFeedback(side) {
    const feedbackElement = document.getElementById(side === 'left' ? 'leftFeedback' : 'rightFeedback');
    const phrase = document.getElementById('phrase');
    
    // Match the current text color
    const currentColor = window.getComputedStyle(phrase).color;
    feedbackElement.style.backgroundColor = currentColor;
...
async function goToPreviousPhrase() {
    if (phraseHistory.length === 0 || currentPhraseIndex <= 0) {
        // No previous phrase available, just get a new random one
        await goToNextPhrase();
        return;
    }
    
    currentPhraseIndex--;
    const previousPhrase = phraseHistory[currentPhraseIndex];
    await displayPhrase(previousPhrase);
}
...
async function goToNextPhrase() {
    ...
    if (currentPhraseIndex < phraseHistory.length - 1) {
        // We have a phrase ahead in history
        currentPhraseIndex++;
        nextPhrase = phraseHistory[currentPhraseIndex];
    } else {
        // Get a new phrase
        nextPhrase = await getPhrase();
        phraseHistory.push(nextPhrase);
        currentPhraseIndex = phraseHistory.length - 1;
        
        // Keep history manageable (max 20 phrases)
        if (phraseHistory.length > 20) {
            phraseHistory.shift();
            currentPhraseIndex--;
        }
    }
    
    await displayPhrase(nextPhrase);
    
    // Restart the auto-timer
    if (pageIsVisible) {
        updateTimer = setTimeout(updateDisplay, 5000); // 5 seconds
    }
}

// Display a specific phrase (extracted from updateDisplay for reuse)
async function displayPhrase(phraseText) {
    if (!pageIsVisible) return;
...
// Update the original updateDisplay to use history tracking
const originalUpdateDisplay = updateDisplay;
updateDisplay = async () => {
    if (!pageIsVisible) return;
    
    // Get new phrase and add to history
    const newPhrase = await getPhrase();
    phraseHistory.push(newPhrase);
    currentPhraseIndex = phraseHistory.length - 1;
